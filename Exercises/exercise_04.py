...
def rc(dna):
    ...
    x = ''.join([comp_dict[char] for char in dna])[::-1]
    return x
...
def percent_decimal(numbers):
    ...
    #     else:
    #         new_numbers.append(num * 100)
    y = [num/100 if num > 1 else num*100 for num in numbers]
...
def multiple_proteins_from_rna(rna):
    ...
    z = ''.join([C2AA[rna[i:i+3]] for i in range(0,len(rna),3)])

    x =[z[index:] for index, char in enumerate(z) if char == "M"]
